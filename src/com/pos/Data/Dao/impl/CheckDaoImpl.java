...
public class CheckDaoImpl implements CheckDao {
    private static final String SQL_TAKE_CHECKS_BY_DATE = "SELECT c.id,c.pos,c.cashier_id,c.check_code,c.sum," +
            "c.date_stamp,c.deleted FROM checks c WHERE ? <= date_stamp AND c.date_stamp <= ? AND c.deleted = false";
    private static final String SQL_INSERT_NOT_ACCEPTED = "INSERT INTO checks (pos,cashier_id,check_code,sum," +
            "date_stamp,deleted) VALUES (?,?,?,?,?,?)";
    private static final String SQL_FIND_BY_ID = "SELECT c.id,c.pos,c.cashier_id,c.check_code,c.sum,c.date_stamp," +
            "c.deleted FROM checks c WHERE c.id=?";
    private static final String SQL_FIND_BY_POS = "SELECT c.id,c.pos,c.cashier_id,c.check_code,c.sum,c.date_stamp," +
            "c.deleted FROM checks c WHERE c.pos=?";
    private static final String SQL_UPDATE = "UPDATE checks SET pos=?,cashier_id=?,check_code=?,sum=?,date_stamp=?," +
            "deleted=? WHERE id=?";
    private static final String SQL_UPDATE_ACCEPTED = "UPDATE checks SET deleted = false WHERE date_stamp=?";
    private static final String SQL_DELETE_CHECK = "UPDATE checks SET deleted=true WHERE id=?";
    ...
    public ArrayList<Check> findCheckByDate(Timestamp startDate, Timestamp endDate) {
        ...
        try (Connection connection = dataBaseManager.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement(SQL_TAKE_CHECKS_BY_DATE);
    ...
    public boolean deleteById(Long id) {
        try (Connection connection = dataBaseManager.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(SQL_DELETE_CHECK);
            statement.setLong(1, id);
            statement.executeUpdate();
            return true;
        } catch (SQLException e){
